...
d = read.table("chr12_110336719_113263518", as.is = T, head = T)
d = d[d$POS > 111600000 & d$POS < 112400000,]
ens = read.table( "../../fig3/refGene_10_16_2014.txt.gz", as.is = T)
tmp = d[d$SNPID == "rs3184504",]


pdf("SH2B3_region.pdf", height= 6, width = 8)
...
par(mar = c(0, 4, 4, 2))
plot(d$POS, -log10( pnorm(-abs(d$Z_HTHY))*2), axes = F, xlab = "", ylab = expression('-log'[10]*'(P) [hypothyroidism]'), pch = 20, col = "black")
points(tmp$POS, -log10( pnorm(-abs(tmp$Z_HTHY))*2), pch = 20, col = "red")
...
par(mar = c(2, 4, 4, 2))
plot(d$POS,  log10( pnorm(-abs(d$Z_HDL))*2), axes = F, xlab = "", ylab = expression('log'[10]*'(P) [HDL]'), ylim = c(-7, 0), pch = 20)
points(tmp$POS, log10( pnorm(-abs(tmp$Z_HDL))*2), pch = 20, col = "red")
...
par(mar = c(5, 1, 4, 1))
d = read.table("rs3184504.pheno2", as.is = T, head = T)
...
plot(NA, xlim = c(-0.2, 0.20), ylim = c(0, 1), axes = F, ylab = "", xlab = "Effect size (s.d./ln[odds])")
axis(1)

mtext("B. rs3184504 T->C, all effects", adj = 0, cex = 0.8)
