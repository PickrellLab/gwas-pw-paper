...
d = read.table("chr2_26894985_28598777", as.is = T, head = T)
d = d[d$POS > 27650000 & d$POS < 27850000,]
ens = read.table( "../../fig3/refGene_10_16_2014.txt.gz", as.is = T)
tmp = d[d$SNPID == "rs1260326",]


pdf("GCKR_region.pdf", height= 6, width = 8)
...
par(mar = c(0, 4, 4, 2))
plot(d$POS, -log10( pnorm(-abs(d$Z_FG))*2), axes = F, xlab = "", ylab = expression('-log'[10]*'(P) [fasting glucose]'), pch = 20, col = "black")
points(tmp$POS, -log10( pnorm(-abs(tmp$Z_FG))*2), pch = 20, col = "red")
...
print(e)
mtext("Position on chr2 (Mb)", side = 1, line = 2.2, cex = 0.8)
for (i in 1:nrow(e)){
	y = i/nrow(e) + 0.1
	mi = min(d$POS)
	ma = max(d$POS)
...
par(mar = c(2, 4, 4, 2))
plot(d$POS,  log10( pnorm(-abs(d$Z_HEIGHT))*2), axes = F, xlab = "", ylab = expression('log'[10]*'(P) [Height]'), ylim = c(-12, 0), pch = 20)
points(tmp$POS, log10( pnorm(-abs(tmp$Z_HEIGHT))*2), pch = 20, col = "red")
axis(2)
...
par(mar = c(5, 1, 4, 1))
d = read.table("rs1260326.pheno2", as.is = T, head = T)
...
plot(NA, xlim = c(-0.15, 0.20), ylim = c(0, 1), axes = F, ylab = "", xlab = "Effect size (s.d./ln[odds])")
axis(1)

mtext("B. rs1260326 T->C, all effects", adj = 0, cex = 0.8)
