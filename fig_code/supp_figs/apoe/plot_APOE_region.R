...
d = read.table("chr19_44744108_46102697", as.is = T, head = T)
#d[abs(d$Z_AD)> 30 & !is.na(d$Z_AD),]$Z_AD = 30
d = d[d$POS > 45300000 & d$POS < 45500000,]
ens = read.table( "../../fig3/refGene_10_16_2014.txt.gz", as.is = T)
tmp = d[d$SNPID == "rs6857",]


pdf("APOE_region.pdf", height= 6, width = 8)
...
par(mar = c(0, 4, 4, 2))
plot(d$POS, -(pnorm(-abs(d$Z_AD), log = T) + log(2))/log(10), axes = F, xlab = "", ylab = expression('-log'[10]*'(P) [Alzheimer\'s]'), pch = 20, col = "black")
points(tmp$POS, -(pnorm(-abs(tmp$Z_AD), log = T) +log(2))/log(10), pch = 20, col = "red")
...
par(mar = c(2, 4, 4, 2))
plot(d$POS,  log10( pnorm(-abs(d$Z_NST))*2), axes = F, xlab = "", ylab = expression('log'[10]*'(P) [nearsightedness]'), ylim = c(-7, 0), pch = 20)
points(tmp$POS, log10( pnorm(-abs(tmp$Z_NST))*2), pch = 20, col = "red")
...
par(mar = c(5, 1, 4, 1))
d = read.table("rs6857.pheno", as.is = T, head = T)
...
mtext("B. rs6857 C->T, all effects", adj = 0, cex = 0.8)
