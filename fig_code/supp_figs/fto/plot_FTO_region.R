...
d = read.table("chr16_53382572_55903774", as.is = T, head = T)
d = d[d$POS > 53500000 & d$POS < 54000000,]
ens = read.table( "../../fig3/refGene_10_16_2014.txt.gz", as.is = T)
tmp = d[d$SNPID == "rs1421085",]


pdf("FTO_region.pdf", height= 6, width = 8)
...
par(mar = c(0, 4, 4, 2))
plot(d$POS, -log10( pnorm(-abs(d$Z_AAM_23))*2), axes = F, xlab = "", ylab = expression('-log'[10]*'(P) [age at menarche]'), pch = 20, col = "black")
points(tmp$POS, -log10( pnorm(-abs(tmp$Z_AAM_23))*2), pch = 20, col = "red")
...
print(e)
mtext("Position on chr12 (Mb)", side = 1, line = 2.2, cex = 0.8)
for (i in 1:nrow(e)){
	y = i/nrow(e) + 0.1
	mi = min(d$POS)
	ma = max(d$POS)
...
par(mar = c(2, 4, 4, 2))
plot(d$POS,  log10( pnorm(-abs(d$Z_CUP))*2), axes = F, xlab = "", ylab = expression('log'[10]*'(P) [breast size]'), ylim = c(-7, 0), pch = 20)
points(tmp$POS, log10( pnorm(-abs(tmp$Z_CUP))*2), pch = 20, col = "red")
...
par(mar = c(5, 1, 4, 1))
d = read.table("rs1421085.phenos", as.is = T, head = T)
...
plot(NA, xlim = c(-0.05, 0.2), ylim = c(0, 1), axes = F, ylab = "", xlab = "Effect size (s.d./ln[odds])")
axis(1)

mtext("B. rs1421085 T->C, all effects", adj = 0, cex = 0.8)
