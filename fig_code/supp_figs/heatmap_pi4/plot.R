...
toplot = read.table("overlap_matrix_pi4", as.is = T)
...
for (i in 1:(length(o))){
	...
	n = gsub("_23", "", n)
	mtext(n, las = 2, side = 1, at = (i-1)*(1/nrow(toplot)) + 0.5/nrow(toplot), cex = 0.7)
	mtext(n, las = 2, side = 2, at = 1-(i-1)*(1/nrow(toplot)) - 0.5/nrow(toplot), cex =0.7)
        for(j in (i):length(o)){
...
mtext("Proportion of nearby signals across all pairs of traits", adj = 0)
par(mar = c(0,0,0,0))
